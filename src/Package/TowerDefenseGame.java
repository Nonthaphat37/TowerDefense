...
public class TowerDefenseGame extends BasicGame{
	...
	//skill
	public int checkClickLastTower = -1;
	...
	public void releaseMonster() throws SlickException{
		if(wave == 1){
			...
			if(wavesmall == 1 && number_monster < max_monsterwave[(wave-1)*3+(wavesmall-1)]){
				...
				if(!monster_checkTotal){
					monsterAll.add(new MonsterLv2MiniBoss(monster_startX, monster_startY));
					monster_checkTotal = true;			// check monster release
					timer_monster = 0;
					number_monster++;
				}
				...
				if(!monster_checkTotal){
					monsterAll.add(new MonsterLv2Boss(monster_startX, monster_startY));
					monster_checkTotal = true;			// check monster release
					timer_monster = 0;
					number_monster++;
				}
			...
			if(wavesmall == 1 && number_monster < max_monsterwave[(wave-1)*3+(wavesmall-1)]){
				if(!monster_checkTotal){
					monsterAll.add(new MonsterLv3(monster_startX, monster_startY));
					monster_checkTotal = true;			// check monster release
					timer_monster = 0;
					number_monster++;
				}
				...
				if(!monster_checkTotal){
					monsterAll.add(new MonsterLv3MiniBoss(monster_startX, monster_startY));
					monster_checkTotal = true;			// check monster release
					timer_monster = 0;
					number_monster++;
				}
				...
				if(!monster_checkTotal){
					monsterAll.add(new MonsterLv3Boss(monster_startX, monster_startY));
					monster_checkTotal = true;			// check monster release
					timer_monster = 0;
					number_monster++;
				}
	...
	public static void releaseBullet(int numTower){
		...
		//decrease mana when use skill release bullet
		if(tower.get(numTower).skillWaterOn && tower.get(numTower).element == 1 && tower.get(numTower).Mana >= 0){
			tower.get(numTower).Mana -= 1;
			if(tower.get(numTower).Mana < -1){
				tower.get(numTower).Mana = -1;
			}
		}
	}
			 ...
			 //Collide
				if(monsterRememberBullet < monsterAll.size()){
					 if(bullet.get(i).get(j).CollideMonster()){
						if(tower.get(i).skillWaterOn){
							monsterAll.get(monsterRememberBullet).getSkillTower = true;
							monsterAll.get(monsterRememberBullet).getskillWaterOn(tower.get(i).element); 
							if(tower.get(i).Mana == 0){
								tower.get(i).Mana = -1;
							}
						}
						
						 bullet.get(i).remove(j);
						 monsterAll.get(monsterRememberBullet).MonsterAttacked(tower.get(i).getAttack(),tower.get(i).getElement(),tower.get(i).skillWaterOn);
	...
	public void drawButtonSkill(Graphics g){
		if(checkClickLastTower != checktowerClick && checkClickLastTower != -1){
			tower.get(checkClickLastTower).getSkill("close");
		}
		if(tower.get(checktowerClick).element != 99){
			tower.get(checktowerClick).getSkill("open");
			checkClickLastTower = checktowerClick;
		}
	}
	...
	public void mouseMoved(int oldx, int oldy, int newx, int newy){
		if(isGameStarted && !isGameOver){
			...
			if(checkMouseClicktower){
				...
				if(newx > 1700 && newx < 1700+78 && newy > 400 && newy < 400+78){
					tower.get(checktowerClick).getMouseMoveSkill(true);
				}
				else{
					tower.get(checktowerClick).getMouseMoveSkill(false);
				}
			}
		}
	}
	...
	public void mouseReleased(int button, int x, int y){
		if(isGameStarted && !isGameOver){
			if(checktowerClick != -1 ){
				if(x > 1700 && x < 1700+78 && y > 400 && y < 400+78){
						tower.get(checktowerClick).skillMouseClick = true;
				}
			}
		}
	}
