...
public class Tower implements Entity {
	...
	protected boolean skillSlow = false;
	...
	protected int speedSecTower = 2500;
	protected int calSpeed = 0;
	...
	// skill
	public boolean skillShow = false;
	public boolean skillWaterOn = false;
	public boolean skillFireOn = false;
	protected boolean skillMouseOver = false;
	protected boolean skillMouseClick = false;
	private int MaxMana = 3;
	protected int Mana = 3;
	private int MaxRegenmana = 10000;
	protected int Regenmana = 10000;
	...
	public void rangeCheck(Monster m, int numMon, int delta) {
		/*
		 * if(Math.sqrt(Math.pow(m.getX()-x, 2) + Math.pow(m.getY()-y, 2)) <=
		 * rangeTower && !checkremember_Mon){ rememberNumMon = numMon;
	...
	private void CalSpeedTower(int delta) {
		...
		// System.out.println(calSpeed + " " + speedrealTower);
	...
	public void getSkill(String skillOpen) {
		if (skillOpen == "open") {
			skillShow = true;
		} else {
			skillShow = false;
	...
	protected void getskillFire() {
		skillFireOn = true;
	}

	public void getMouseMoveSkill(boolean SkillMouseOver) {
		skillMouseOver = SkillMouseOver;
	}

	protected void RegenMana(int delta) {
		if (Mana < MaxMana) {
			Regenmana -= delta;
			if (Regenmana <= 0) {
				if (Mana == -1) {
